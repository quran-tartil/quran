...
class TopicCategorySeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        TopicCategory::truncate();
        Schema::enableForeignKeyConstraints();
   
        // get data from csv file
        $csvFile = fopen(base_path("database/data/topicCategories.csv"), "r");
        $firstline = true;
        $i = 0;
        ...
        foreach ($actions as $action) {
            $permissionName = $action . '-' . "TopicCategoryController";
            Permission::create(['name' => $permissionName, 'guard_name' => 'web']);
        }

        $memberRolePermissions = [
            "index-TopicCategoryController",
            "show-TopicCategoryController",
        ];

        $adminManagerRolePermissions = [
            'index-TopicCategoryController',
            'show-TopicCategoryController',
            'create-TopicCategoryController',
            'store-TopicCategoryController',
            'edit-TopicCategoryController',
            'update-TopicCategoryController',
            'destroy-TopicCategoryController',
            'export-TopicCategoryController',
            'import-TopicCategoryController'
        ];
